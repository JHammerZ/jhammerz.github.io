cat << 'EOF' > scripts/terminal_orchestrator.sh
#!/usr/bin/env bash
# ==============================================================================
...
chmod +x scripts/terminal_orchestrator.sh
cp scripts/terminal_orchestrator.sh orchestrator
chmod +x orchestrator

# Install to global path if running on Termux or Linux
if [ -n "${PREFIX:-}" ] && [ -d "${PREFIX}/bin" ]; then
    cp orchestrator "${PREFIX}/bin/orchestrator"
    chmod +x "${PREFIX}/bin/orchestrator"
elif [ -d "$HOME/.local/bin" ]; then
    cp orchestrator "$HOME/.local/bin/orchestrator"
    chmod +x "$HOME/.local/bin/orchestrator"
fi