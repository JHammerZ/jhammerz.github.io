...
# Creating an unprivileged IPv6 TCP Socket Listener
try:
    server = socket.socket(socket.AF_INET6, socket.SOCK_STREAM)
    server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    # Bind directly to all local interfaces on an open mesh testing port
    server.bind(('::', 9999))
    server.listen(5)
    print("  -> \033[1;32m[ONLINE]\033[0m Mesh Server actively listening on port \033[1;33m9999\033[0m completely offline.")
    print("  -> Tell neighbor nodes to run: nc -6 [your_mesh_ip] 9999")
...
try:
    ...
    print("\n[*] Chat server suspended back to core memory storage channels.")
finally:
    server.close()
